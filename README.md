Release notes for conflict parser 0.1.0

Conflict parser is a rust program that consists of three files: lib.rs, main.rs and demo.txt. It consumes a file of transactions in MySQL flavor
like those in demo.txt and outputs the possible conflicts as in Example 16 for further analysis. After downloading these files, create a rust 
project named 'conflict_parser' and place the first two files in the 'src' directory as usual. The third file goes to the project home, namely, 
the 'conflict_parser' directory you just created. This file contains the TPCC application for testing purpose. In the future when you want to 
run the program against the real-world application of yours, convert the transactions into a file like 'demo.txt' with the guidelines presented next and 
place it in the project home. For those who are not familiar with rust, the following is a starting point:

https://doc.rust-lang.org/book/

You only need to read the first chapter to run it.

Conflict parser depends on the 'regex' package, please insert the following line to the 'dependencies' section in your Cargo.toml file
...
where input.txt is a file like demo.txt that represents your application
...
I didn't fire up a state machine for it. Instead, I've used simple regular expressions for that purpose. This implies the following rules for the input
file syntax:
