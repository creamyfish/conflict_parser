...
//This function takes transactions generated by the parse function and resolve them into RW conflicts, which are printed out as in Example 16
fn resolve(trans: &Vec<Transaction>) {
    ...
    for t in trans {
        for s in &t.statements {
            //Process the single field write in an update statement for items
            if s.kind == "update" {
                let ta = items.entry(s.table.clone()).or_insert(Vec::new());
                ...
                //Process the single field write in an update for predicates  
                let ta = predicates.entry(s.table.clone()).or_insert(Vec::new());
                let length = ta.len();
                let mut i = 0;
    ...
    //Print the RW conflicts per table as in Example 16
...
//This function prints the RW conflicts per transaction pair as in Example 16
...
//This function prints the RW conflicts per table as in Example 16
fn print_rw(it: &HashMap<String, Vec<ReadWriteTransactionsItem>>, pr: &HashMap<String, Vec<ReadWriteTransactionsPred>>) {
    ...
    for (key, value) in it {
        ...
        //As opposed to Example 16, only RW conflicts are printed
        if present == true {
            println!("Table: {}\n",  key);
